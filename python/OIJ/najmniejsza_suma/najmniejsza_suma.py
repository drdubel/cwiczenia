def najmniejsza_suma(cyfry, il_liczb):
    ...
    wynik += sum(cyfry_bez_zer[:il_liczb_dluzszych]) * 10**dlugosc_liczb
    il_zer = cyfry.count(0)
    cyfry_bez_zer = cyfry_bez_zer[il_liczb_dluzszych:]
    wynik += sum(cyfry_bez_zer[: il_liczb - il_liczb_dluzszych]) * 10 ** (
        dlugosc_liczb - 1
    )
    ...
    for cyfra in cyfry_bez_zer:
        cyfry.append(cyfra)
    wynik += sum(cyfry[:il_liczb_dluzszych]) * 10 ** (dlugosc_liczb - 1)
